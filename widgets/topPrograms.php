 ...
 * File: topPrograms.php
 * 
 * Top programs widget, to view a list of top programs
 * 
 */

class TopVifeWidget extends WP_Widget
{
  function TopVifeWidget()
  {
    $widget_ops = array('classname' => 'TopVifeWidget', 'description' => __('Deze widget toont je beste programma&rsquo;s op basis van je eigen eCPC. Je kan zelf bepalen hoeveel programma&rsquo;s je toont.','DaisyconPlugin') );
    $this->WP_Widget('TopwVifeWidget', __('Daisycon populairste','DaisyconPlugin'), $widget_ops);
  }
...
<?php
  ...
  public function widget($args, $instance)
  {
	...
	echo '	<script type="text/javascript">					
				function insertStats4(){
					jQuery.ajax
					({
						url: "http://developers.affiliateprogramma.eu/wordpressplugin/json.php?method=insertClicks&website='.base64_encode($_SERVER['SERVER_NAME']).'&mediaid='.$wi[0].'&object=widget_topprograms&jsoncallback=?",
						dataType: "jsonp",
						cache: false,
	...
	$results = $wpdb->get_results("SELECT 	programs.name,
											programs.image,
											programs.url FROM programs INNER JOIN categories ON programs.category = categories.name WHERE programs.visible = '1' AND categories.visible = '1' ORDER BY programs.ecpc DESC LIMIT ".$instance['aantal']."");
	...
	foreach($results as $array){
		...
		if ($instance['image'] == 1){
			echo	'
					<a href="'.$array->url.'" target="_blank" rel="nofollow" onclick="insertStats4()">
