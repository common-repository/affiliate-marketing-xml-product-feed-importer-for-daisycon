 ...
 * File: newPrograms.php
 * 
 * New programs widget, to view a list of new programs
 * 
 */

class NewVifeWidget extends WP_Widget
{
  function NewVifeWidget()
  {
    $widget_ops = array('classname' => 'NewVifeWidget', 'description' => __('Deze widget toont de nieuwste affiliateprogramma&rsquo;s. Je kan zelf bepalen hoeveel programma&rsquo;s je toont.','DaisyconPlugin') );
    $this->WP_Widget('NewVifeWidget', __('Daisycon nieuwste','DaisyconPlugin'), $widget_ops);
  }
...
<?php
  ...
  public function widget($args, $instance)
  {
	...
	echo '	<script type="text/javascript">
				function insertStats2(){
					jQuery.ajax
					({
						url: "http://developers.affiliateprogramma.eu/wordpressplugin/json.php?method=insertClicks&website='.base64_encode($_SERVER['SERVER_NAME']).'&mediaid='.$wi[0].'&object=widget_newprograms&jsoncallback=?",
						dataType: "jsonp",
						cache: false,
	...
	$results = $wpdb->get_results("SELECT 	programs.name,
											programs.image,
											programs.url FROM programs INNER JOIN categories ON programs.category = categories.name WHERE programs.visible = '1' AND categories.visible = '1' ORDER BY date LIMIT ".$instance['aantal']."");
	...
	foreach($results as $array){
		...
		if($instance['image'] == 1){
			echo	'
					<a href="'.$array->url.'" target="_blank" rel="nofollow" onclick="insertStats2()">
